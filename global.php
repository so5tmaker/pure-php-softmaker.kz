...
function show_title($cat_name, $mode = 1, $fname = '') {
    ...
    if ($mode == 0) {
        echo "<p><strong>".get_foreign_equivalent("Вы можете прочесть эту же статью на другом языке").":</strong></p>";
    } elseif ($mode == 1) {
        $cat_nm = get_fld_by_id($cat, 'categories', 'name');
        $path = "$rest_/$fnvalue/$cat_nm/";
        $cat_name = "<a $open href='$path'>$cat_name</a>";
        echo "<p><strong>".get_foreign_equivalent("Вы также можете прочитать похожие статьи из текущей категории")." ".$cat_name.":</strong></p>";
    } elseif ($mode == 2) {
        $cat_nm = $cat_name[0];
        $cat_name = $cat_name[1];
        $path = "$rest_/$fnvalue/$cat_nm/";
        $cat_name = "<a $open href='$path'>$cat_name</a>";
        echo "<p><strong>".get_foreign_equivalent("Вы также можете прочитать статьи из соседней категории")." ".$cat_name.":</strong></p>";
    }elseif ($mode == 3) {
        $path = "$rest_/$fnvalue/";
        $cat_name = "<a $open href='$path'>$cat_name</a>";
        echo "<p><strong>".get_foreign_equivalent("Вы также можете прочитать статьи из соседнего раздела")." ".$cat_name.":</strong></p>";
    }
}
...
//    echo '<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >';
    ...
    echo $str.'</ol></div><p class="clear"></p>';
...
//        $filename = str_replace(".php", "", $sec_row["file"]);
    ...
    if ($num_rows != 0) { 
        ...
        $num_rows = mysql_num_rows($res_cat);
        if ($num_rows != 0) {
            show_title($sec_name, 3, $filename);
        }
        do 
        {
...
function get_query($num_elements, $sign, $order = 'ASC') {
    global $id_score, $cat, $lang, $filename, $tbl;
    $faq = "";
    if ($filename == "articles"){
//        $faq = "AND(faq='0')";
    }
    return "SELECT text,title,id,cat,name,mini_img,cat,meta_d FROM ".$tbl." WHERE (cat=$cat)".$faq."AND(lang='".$lang."')AND(id".$sign.$id_score.") ORDER BY id ".$order." LIMIT 0 , ".$num_elements;
}
...
function get_query_cat($sign, $order = 'ASC') {
    global $sec, $cat, $lang;
    return "SELECT title,id,name FROM categories WHERE (id".$sign.$cat.")AND(lang='".$lang."')AND(sec='".$sec."') ORDER BY id ".$order." LIMIT 0 , 1";
}
...
//    return "SELECT title,id FROM categories WHERE (lang='".$lang."')AND(sec'".$sign.$sec."') ORDER BY id ".$order." LIMIT 0 , 1";
      return "SELECT text,title,id,cat_tbl FROM categories WHERE (lang='".$lang."')AND(sec".$sign.$sec.") ORDER BY id ".$order;
}
...
function get_highlight_code($code) 
{ 
    $string = $code;
...
function get_cat_rows($user) {
    ...
    $db_class = new DB();
    $cat_array = $db_class->select('categories', "lang='$user->lang'", "id, title");
//    $resultArray = array();
...
function get_all_cat_rows() {
    ...
    $db_class = new DB();
    $cat_array = $db_class->select('categories', "lang='$lang'", "id, title");
    $cat_rows = '';
    foreach($cat_array as $k => $v) {
        $cat_rows .= 
        "<tr>
