<? include ("lock.php");?>
<? $name_dt = "словарной статьи"; $tbl_dt = "dic"; $action = "dic.php";
...
if (isset($_GET['mode'])){// 1-я часть 3 метода GET
    if ($_GET['mode'] == 'new') {// добавление нового значения
       ...
       <form name="form1" method="post" action="<? echo $action ?>">
         <p>
           <label>Слово казахской <? echo $name_dt ?><br>
             <textarea name="KZ" id="KZ" cols="<? echo $ColsOfarea ?>" rows="5"></textarea>
           </label>
         </p>
         <p>
        ...
        <?
            ...
            if (!isset($id))
            {
                ...
                do 
                {
                    printf ("<p><a href='%s?id=%s&mode=edit'>%s</a></p>",$action,$myrow["id"],$myrow["KZ"]." - ".$myrow["RU"]." - ".$myrow["EN"]);
                }
                ...
                <form name="form1" method="post" action="<? echo $action ?>">
                 <p>
                   <label>Слово казахской <? echo $name_dt ?><br>
                     <textarea name="KZ" id="KZ" cols="<? echo $ColsOfarea ?>" rows="5"><?echo $myrow['KZ']?></textarea>
                   </label>
                 </p>
                 <p>
                   <label>Слово русской <? echo $name_dt ?><br>
            ...
            do 
            {
                printf ("<p><input name='id' type='radio' value='%s'><label> %s</label></p>",$myrow["id"],$myrow["KZ"]." - ".$myrow["RU"]." - ".$myrow["EN"]);
            }
            while ($myrow = mysql_fetch_array($result));
            ?>
 ...
 if (isset($_POST['mode'])){// 2-я часть 3 метода POST
    if ($_POST['mode'] == 'add') {// добавление нового значения
        $title_here = "Страница добавления ".$name_dt; include("header.html");
        if (isset($_POST['KZ'])) {$KZ = $_POST['KZ']; if ($KZ == '') {unset($KZ);}}
        if (isset($_POST['RU']))  {$RU = $_POST['RU'];   if ($RU == '' ) {unset($RU); }}
        if (isset($_POST['EN']))  {$EN = $_POST['EN'];   if ($EN == '' ) {unset($EN); }}
        if (isset($KZ) && isset($RU) && isset($EN))
        {
            $id = get_id($tbl_dt);
            $result = mysql_query ("INSERT INTO ".$tbl_dt." (id,KZ,RU,EN) VALUES ($id, '$KZ', '$RU', '$EN')");
        ...
        if (isset($KZ) && isset($RU) && isset($EN)){
        $result = mysql_query ("UPDATE ".$tbl_dt." SET KZ='$KZ', RU='$RU', EN='$EN'  WHERE id='$id'");            
        if ($result == 'true') {
