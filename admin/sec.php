<? include ("lock.php");?>
<? $name_dt = "раздела"; $tbl_dt = "sections"; $action = "sec.php";
$translit = TRUE;
...
if (isset($_GET['mode'])){// 1-я часть 3 метода GET
    if ($_GET['mode'] == 'new') {// добавление нового значения
        ...
        <form name="form1" method="post" action="<? echo $action ?>">
         <p>
           <label>Введите название <? echo $name_dt ?><br>
             <input 
                    type="text" name="title" id="title" size="<? echo $SizeOfinput ?>">
           </label>
         </p>
         <p>
         ...
         <p>
           ...
           <input type="text" name="file" id="file" maxlength="40" size="<? echo $SizeOfinput ?>">
           </label>
         </p>
         <p>
        ...
        <?
            ...
            if (!isset($id))
            {
                $result = mysql_query("SELECT title,id,lang FROM ".$tbl_dt);
                ...
                <form name='form1' method='post' action='$action'>
                         ...
                         <p>
                           <label>Введите полный текст описания $name_dt<br>
                           <textarea name="text" id="text" cols="$ColsOfarea" rows="20">$myrow[text]</textarea>
                           </label>
                         </p>

                         <p>
                           <label>Введите полный путь к файлу $name_dt<br>
...
HERE;
          ...
          ?>
                ...
                <form action="<? echo $action;?>" method="post">
                    ...
                    do
                    {
                    printf ("<p align='center'><input name='id' type='radio' value='%s'><label> %s</label></p>",$myrow["id"],$myrow["title"]);
                    }
                    while ($myrow = mysql_fetch_array($result));
                    ?>
 ...
 if (isset($_POST['mode'])){// 2-я часть 3 метода POST
    if ($_POST['mode'] == 'add') {// добавление нового значения
        ...
        if (isset($title) && isset($name) && isset($meta_d) && isset($meta_k) && isset($text)&& isset($file))
        {
        /* Здесь пишем что можно заносить информацию в базу */
        $id = get_id($tbl_dt);
        $result = mysql_query ("INSERT INTO ".$tbl_dt." (id,id_lang,name,title,meta_d,meta_k,text,file,lang) VALUES ($id,'$id_lang','$name','$title','$meta_d','$meta_k','$text','$file','$lang')");
        ...
        {
            /* Здесь пишем что можно заносить информацию в базу */
            $result = mysql_query ("UPDATE ".$tbl_dt." SET name='$name',title='$title', meta_d='$meta_d', meta_k='$meta_k', text='$text', file='$file', lang='$lang', id_lang='$id_lang' WHERE id='$id'");
            if ($result == 'true') {
        ...
        $title_here = "Страница удаления ".$name_dt; include("header.html");
        if (isset($id)){
            $result0 = mysql_query ("SELECT id FROM categories WHERE sec='$id'",$db);
